...
function M.load(onMenu, onFeedback)
  ...
  _btnFeedback = Button.new(x, y2, BTN_W * SCALE_X, BTN_H * SCALE_Y, "FEEDBACK", FONT_SIZE, onFeedback)
end
...
function M.mousepressed(mx, my, btn)
  return _btnMenu:mousepressed(mx, my, btn) or _btnFeedback:mousepressed(mx, my, btn)
end

function M.touchpressed(tx, ty)
  return _btnMenu:touchpressed(tx, ty) or _btnFeedback:touchpressed(tx, ty)
end
